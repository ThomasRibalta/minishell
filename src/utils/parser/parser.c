...
/*   Updated: 2024/06/29 03:03:14 by jedurand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_astnode	*build_command_pipe_tree(t_token **current_token)
{
	...
	{
		if (process_token(current_token, &root, &current_command,
				&temp_inputs, &temp_outputs))
			break ;
		*current_token = (*current_token)->next;
	}
	finalize_command(&root, &current_command, &temp_inputs, &temp_outputs);
	return (root);
}
