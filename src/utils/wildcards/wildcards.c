...
char **check_full_wildcard(char **tab_current_file, int i)
{
    ...
    if (tab_current_file2)
        tab_current_file = fusionner_tableaux(tab_current_file, tab_current_file2, i);
    ...
    while (tab_current_file[i])
    {
        while(tab_current_file[i][++j])
        {
            if (only_char(tab_current_file[i], '*') == ft_strlen(tab_current_file[i]) && tab_current_file[i][0] == '*')
                ...
                tab_current_file = check_middle_wildcard(tab_current_file, i, j);
            if ((only_char(tab_current_file[i], '*') == ft_strlen(tab_current_file[i]) && tab_current_file[i][0] == '*')||
            (tab_current_file[i][j] == '*' && j == 0) || tab_current_file[i][j] == '*')
                break;
        }
        j = -1;
        i++;
